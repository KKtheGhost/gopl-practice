...
func (ic IssueCache) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		_, err := w.Write([]byte(fmt.Sprintf("Issue number is not a number: '%s'", numStr)))
		if err != nil {
			log.Printf("Error writting response for %s: %s", r, err)
		}
		return
	}
	...
	logNonNil(issueListTemplate.Execute(w, issue))
}
